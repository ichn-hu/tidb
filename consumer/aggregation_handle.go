...
func (aggHandler *AggFuncHandler) getAvg() float64 {
	return float64(aggHandler.sum) / float64(aggHandler.count)
}
...
func (aggHandler *AggFuncHandler) getDistinctAvg() float64 {
	return float64(aggHandler.distinctSum) / float64(len(aggHandler.occurs))
}
