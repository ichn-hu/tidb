...
type MultiwayHashJoinExec struct {
	baseExecutor
	/*
		joinExecutors are the executors generated by optimizer in a left-deep manner
		        j[2]
		       /   \
			j[1]  b[2]
		     / \
...
func (e *MultiwayHashJoinExec) fetchAndProbeHashTable(ctx context.Context) {
	...
	for i := range e.probeKeys {
		probeKeyColIdx[i] = make([]int, len(e.probeKeys[i]))
		for j := range e.probeKeys[i] {
			probeKeyColIdx[i][j] = e.probeKeys[i][j].Index
		}
	}
...
func (e *MultiwayHashJoinExec) runJoinWorker(workerID uint, probeKeyColIdx [][]int) {
	...
	hCtxs := make([]*hashContext, e.numBuildExec)
	for i := 0; i < e.numBuildExec; i++ {
		hCtxs[i] = &hashContext{
			allTypes:  e.probeTypes[i],
			keyColIdx: probeKeyColIdx[i],
		}
	}
	for ok := true; ok; {
		if e.finished.Load().(bool) {
		...
		if e.useOuterToBuild {
			...
			ok, joinResult = e.join2Chunk(workerID, probeSideResult, hCtxs, joinResult, selected)
		}
		if !ok {
			break
		}
...
func (e *MultiwayHashJoinExec) join2Chunk(workerID uint, probeSideChk *chunk.Chunk, hCtxs []*hashContext,
	...
	for i := range selected {
		if selected[i] {
			for b, rc := range e.rowContainer {
				matched, _, err := rc.GetMatchedRowsAndPtrs(hCtxs[b].hashVals[i].Sum64(), probeSideChk.GetRow(i), hCtxs[b])
